...
var keysPressed=[];
var count=0;
var wholeNote, halfNote, quarterNote,eightNote,erase;
var noteTime;
var x='e';
var y='eightNote';
var j;
...
if(count%20!==0){
fill(255);
text(keysPressed, h,booty+LW+30);
j=0;}
else if(count!=0&&j==0) {
    h-=innerWidth/2.5;
    j++;
...
function buttons(){
      ...
      wholeNote= createButton('WholeNote');
      wholeNote.position(50, 0);
      wholeNote.mousePressed(WholeN);
      halfNote= createButton('HalfNote');
      halfNote.position(150,0);
      halfNote.mousePressed(HalfN);
      quarterNote= createButton('QuarterNote');
      quarterNote.position(245,0);
      quarterNote.mousePressed(quarterN);
      eightNote= createButton('EightNote');
      eightNote.position(350,0);
      eightNote.mousePressed(eightN);
      text('Your current note is '+y, 450, 25);
      erase= createButton('Delete last Note');
      erase.position(innerWidth/4, innerHeight/2.15);
      erase.mousePressed(removed);
      fin= createButton('FIN');
      fin.position(innerWidth*3/4, innerHeight/2.15);
      fin.mousePressed(end);
}
function end(){
trap();
}
function trap() {
 const   writer= createWriter('Finished');
 writer.print(keysPressed);
   writer.close();
   writer.clear();
}
